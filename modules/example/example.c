...
{
	return module_example_init(boing, module);
}

int module_destroy(boing_t *boing, boing_module_t *module)
{
	return module_example_destroy(boing, module);
}

int module_stack_add(boing_t *boing, boing_value_t *stack, boing_module_t *module)
{
	return module_example_stack_add(boing, stack, module);
}
#endif
...
{
	boing_error(boing, 0, "hello from the example module");
	return 0;
}

int module_example_destroy(boing_t *boing, boing_module_t *module)
{
	boing_error(boing, 0, "goodbye from the example module");
	return 0;
}

int module_example_stack_add(boing_t *boing, boing_value_t *stack, boing_module_t *module)
{
	boing_error(boing, 0, "stack in example module");

	BOING_ADD_GLOBAL("EXAMPLE_GLOBAL", boing_value_from_str(boing, "global that came from the example module"));
